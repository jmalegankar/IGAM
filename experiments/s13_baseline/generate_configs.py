...
# e3b_rand (frozen random φ) AND e3b_idm (inverse-dynamics-LEARNED φ, the
# canonical Henaff 2022 E3B) are both run so the random-vs-learned φ contrast is
# a measured ablation — e3b_rand scored only 1/3 and the φ quality, not λ, is the
# suspected bottleneck.
EXPLORATION_METHODS = ["rnd", "noveld", "e3b_rand", "e3b_idm"]
...
#
# E3B is swept across all cells too, but with the IMPROVED learned-φ variant
# (e3b_idm — canonical Henaff 2022, φ trained by an inverse-dynamics model), NOT
# e3b_rand. e3b_rand scored only 1/3 in the selection arm because a frozen random
# φ is a weak novelty signal; IDM-φ captures controllable state, which is the
# fair "is E3B actually competitive?" test. So the all-cells winner sweep runs
# {noveld, e3b_idm}. GRU/Memoryless × these already exist from the selection arm
# and are deduped.
BEST_METHOD = "noveld"            # kept for back-compat / docs
WINNER_METHODS = ["noveld", "e3b_idm"]

# Lambda-intrinsic probe (anchor sensitivity sweep). The bonus is a SINGLE
# merged reward stream (ppo.py:297: reward += lambda*bonus), std-normalized to
# O(1)/step and NON-annealing, integrated under gamma=0.999 (~1000-step horizon).
# So the discounted intrinsic return already dominates S13's sparse terminal
# (~0.5) by ~10x at lambda=0.01 and would hit ~100x at lambda=0.1 -- i.e. 0.1
# over-explores by construction. We bracket the chosen 0.01 by 3x each way on the
# anchor (GRU+NovelD) to confirm it isn't accidentally mis-set. lambda=0.01 is
# NOT re-emitted here -- it is the existing s13_GRU_noveld_10M.yaml run (reuse
# that wandb curve as the sweep midpoint).
LAMBDA_PROBE_CELL = "GRU"
LAMBDA_PROBE_METHOD = "noveld"
LAMBDA_PROBE_VALUES = [0.003, 0.03]   # 0.01 = existing baseline-arm run
...
def _lam_tag(lam: float) -> str:
    """Bare filesystem-safe lambda tag: 0.003 -> '0p003', 0.03 -> '0p03'.
    Call sites prefix 'lam' for filenames (s13_..._lam0p003_10M.yaml)."""
...
# Lambda-probe per-condition launcher. Same as _EXPLORE_SCRIPT_TEMPLATE but the
# config path + TAG carry the lambda tag, and the file is named
# explore_<cell>_<method>_lam<tag>.sh (still explore_* so launch-jobs.sh / the
# ONLY filter pick it up automatically).
_PROBE_SCRIPT_TEMPLATE = """\
#!/usr/bin/env bash
# AUTO-GENERATED by generate_configs.py — regenerate, don't hand-edit.
# Lambda-intrinsic probe: __CELL__ + __INTRINSIC__ @ lambda=__LAMTAG__ on
# MiniGrid-MemoryS13, all seeds AT ONCE (parallel). Same env knobs as run_*.sh.
set -euo pipefail
...
PARALLEL="${PARALLEL:-1}"
EXTRA="${EXTRA:-}"
CFG="$HERE/../configs/s13___CELL_____INTRINSIC___lam__LAMTAG_____SUFFIX__.yaml"
LOGDIR="$HERE/../_logs"; mkdir -p "$LOGDIR"
TAG="__CELL_____INTRINSIC___lam__LAMTAG__"
...
def write_probe_scripts(cell: str, method: str, lambdas: list[float],
                        suffix: str) -> list[Path]:
    """One explore_<cell>_<method>_lam<tag>.sh per probe lambda."""
    scripts_dir = HERE / "scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)
    written = []
    for lam in lambdas:
        tag = _lam_tag(lam)                       # "0p003"
        body = (_PROBE_SCRIPT_TEMPLATE
                .replace("__CELL__", cell)
                .replace("__INTRINSIC__", method)
                .replace("__LAMTAG__", tag)
                .replace("__SUFFIX__", suffix))
        path = scripts_dir / f"explore_{cell}_{method}_lam{tag}.sh"
        path.write_text(body)
        os.chmod(path, 0o755)
        written.append(path)
    return written
...
def main() -> None:
    ...
    winner_conditions = [(cell, m) for cell in CELLS for m in WINNER_METHODS]
    explore_conditions = []
    ...
    # ── Lambda-intrinsic probe: GRU × NovelD × {0.003, 0.03} ────────────────
    # (lambda=0.01 already ran as s13_GRU_noveld_10M.yaml — the sweep midpoint.)
    probe_cfgs = []
    for lam in LAMBDA_PROBE_VALUES:
        cfg = build_cfg(LAMBDA_PROBE_CELL, intrinsic=LAMBDA_PROBE_METHOD,
                        lambda_intrinsic=lam)
        tag = _lam_tag(lam)
        path = out_dir / (f"s13_{LAMBDA_PROBE_CELL}_{LAMBDA_PROBE_METHOD}"
                          f"_lam{tag}_{suffix}.yaml")
        with open(path, "w") as f:
            yaml.safe_dump(cfg, f, sort_keys=False)
        probe_cfgs.append(path)
    probe_scripts = write_probe_scripts(LAMBDA_PROBE_CELL, LAMBDA_PROBE_METHOD,
                                        LAMBDA_PROBE_VALUES, suffix)
    ...
    for p in expl_scripts:
        print(f"  {p.name}")
    print(f"\nWrote {len(probe_cfgs)} lambda-probe configs + "
          f"{len(probe_scripts)} scripts:")
    for p in probe_cfgs + probe_scripts:
        print(f"  {p.name}")
