...
# Run SHM on MiniGrid-MemoryS13 for its seeds, ALL AT ONCE (parallel).
# Knobs (env vars):
...
PARALLEL="${PARALLEL:-1}"
EXTRA="${EXTRA:-}"
CFG="$HERE/../configs/s13_SHM_none_10M.yaml"
LOGDIR="$HERE/../_logs"; mkdir -p "$LOGDIR"

echo "[SHM] python=$PYTHON device=$DEVICE seeds=[$SEEDS] parallel=$PARALLEL"
pids=()
for s in $SEEDS; do
  if [[ "$PARALLEL" == "1" ]]; then
    "$PYTHON" -m train --config "$CFG" --seed "$s" \
        --runs-dir "$RUNS_DIR" --device "$DEVICE" $EXTRA \
        > "$LOGDIR/SHM_seed${s}.log" 2>&1 &
    pids+=("$!")
    echo "  launched SHM seed=$s (pid $!) → $LOGDIR/SHM_seed${s}.log"
  else
    echo "  SHM seed=$s (sequential)"
    "$PYTHON" -m train --config "$CFG" --seed "$s" \
        --runs-dir "$RUNS_DIR" --device "$DEVICE" $EXTRA
  fi
done
if [[ "$PARALLEL" == "1" ]]; then
  ...
  [[ "$fail" == "0" ]] || { echo "[SHM] a seed FAILED — see $LOGDIR"; exit 1; }
fi
echo "[SHM] all seeds done."
