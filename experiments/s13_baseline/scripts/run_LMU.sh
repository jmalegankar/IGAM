...
# Run LMU on MiniGrid-MemoryS13 for its seeds, ALL AT ONCE (parallel).
# Knobs (env vars):
...
PARALLEL="${PARALLEL:-1}"
EXTRA="${EXTRA:-}"
CFG="$HERE/../configs/s13_LMU_none_10M.yaml"
LOGDIR="$HERE/../_logs"; mkdir -p "$LOGDIR"

echo "[LMU] python=$PYTHON device=$DEVICE seeds=[$SEEDS] parallel=$PARALLEL"
pids=()
for s in $SEEDS; do
  if [[ "$PARALLEL" == "1" ]]; then
    "$PYTHON" -m train --config "$CFG" --seed "$s" \
        --runs-dir "$RUNS_DIR" --device "$DEVICE" $EXTRA \
        > "$LOGDIR/LMU_seed${s}.log" 2>&1 &
    pids+=("$!")
    echo "  launched LMU seed=$s (pid $!) → $LOGDIR/LMU_seed${s}.log"
  else
    echo "  LMU seed=$s (sequential)"
    "$PYTHON" -m train --config "$CFG" --seed "$s" \
        --runs-dir "$RUNS_DIR" --device "$DEVICE" $EXTRA
  fi
done
if [[ "$PARALLEL" == "1" ]]; then
  ...
  [[ "$fail" == "0" ]] || { echo "[LMU] a seed FAILED — see $LOGDIR"; exit 1; }
fi
echo "[LMU] all seeds done."
