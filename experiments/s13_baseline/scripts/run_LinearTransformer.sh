...
# Run LinearTransformer on MiniGrid-MemoryS13 for its seeds, ALL AT ONCE (parallel).
# Knobs (env vars):
...
PARALLEL="${PARALLEL:-1}"
EXTRA="${EXTRA:-}"
CFG="$HERE/../configs/s13_LinearTransformer_none_10M.yaml"
LOGDIR="$HERE/../_logs"; mkdir -p "$LOGDIR"

echo "[LinearTransformer] python=$PYTHON device=$DEVICE seeds=[$SEEDS] parallel=$PARALLEL"
pids=()
for s in $SEEDS; do
  if [[ "$PARALLEL" == "1" ]]; then
    "$PYTHON" -m train --config "$CFG" --seed "$s" \
        --runs-dir "$RUNS_DIR" --device "$DEVICE" $EXTRA \
        > "$LOGDIR/LinearTransformer_seed${s}.log" 2>&1 &
    pids+=("$!")
    echo "  launched LinearTransformer seed=$s (pid $!) → $LOGDIR/LinearTransformer_seed${s}.log"
  else
    echo "  LinearTransformer seed=$s (sequential)"
    "$PYTHON" -m train --config "$CFG" --seed "$s" \
        --runs-dir "$RUNS_DIR" --device "$DEVICE" $EXTRA
  fi
done
if [[ "$PARALLEL" == "1" ]]; then
  ...
  [[ "$fail" == "0" ]] || { echo "[LinearTransformer] a seed FAILED — see $LOGDIR"; exit 1; }
fi
echo "[LinearTransformer] all seeds done."
