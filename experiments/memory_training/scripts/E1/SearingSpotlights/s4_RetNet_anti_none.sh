#!/usr/bin/env bash
# AUTO-GENERATED (E1). RetNet/anti/none seed 4 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/SearingSpotlights/RetNet_anti_none.yaml"
echo "[E1] RetNet/anti/none seed=4"
exec "$PYTHON" -m train --config "$CFG" --seed 4 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/RetNet_anti_none_seed4.log"
