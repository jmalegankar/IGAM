#!/usr/bin/env bash
# AUTO-GENERATED (E1). Memoryless/anti/e3b_idm seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/SearingSpotlights/Memoryless_anti_e3b_idm.yaml"
echo "[E1] Memoryless/anti/e3b_idm seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Memoryless_anti_e3b_idm_seed1.log"
