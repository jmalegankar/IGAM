#!/usr/bin/env bash
# AUTO-GENERATED (E1). GRU/penalty/e3b_idm seed 9 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MysteryPath/GRU_penalty_e3b_idm.yaml"
echo "[E1] GRU/penalty/e3b_idm seed=9"
exec "$PYTHON" -m train --config "$CFG" --seed 9 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GRU_penalty_e3b_idm_seed9.log"
