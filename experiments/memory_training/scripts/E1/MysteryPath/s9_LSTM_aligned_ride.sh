#!/usr/bin/env bash
# AUTO-GENERATED (E1). LSTM/aligned/ride seed 9 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MysteryPath/LSTM_aligned_ride.yaml"
echo "[E1] LSTM/aligned/ride seed=9"
exec "$PYTHON" -m train --config "$CFG" --seed 9 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LSTM_aligned_ride_seed9.log"
