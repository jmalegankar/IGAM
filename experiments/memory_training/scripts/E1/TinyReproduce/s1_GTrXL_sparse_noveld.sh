#!/usr/bin/env bash
# AUTO-GENERATED (E1). GTrXL/sparse/noveld seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/TinyReproduce/GTrXL_sparse_noveld.yaml"
echo "[E1] GTrXL/sparse/noveld seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GTrXL_sparse_noveld_seed1.log"
