#!/usr/bin/env bash
# AUTO-GENERATED (E1). LinearTransformer/sparse/none seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/TinyReproduce/LinearTransformer_sparse_none.yaml"
echo "[E1] LinearTransformer/sparse/none seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LinearTransformer_sparse_none_seed1.log"
