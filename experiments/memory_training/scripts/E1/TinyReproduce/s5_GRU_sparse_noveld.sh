#!/usr/bin/env bash
# AUTO-GENERATED (E1). GRU/sparse/noveld seed 5 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/TinyReproduce/GRU_sparse_noveld.yaml"
echo "[E1] GRU/sparse/noveld seed=5"
exec "$PYTHON" -m train --config "$CFG" --seed 5 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GRU_sparse_noveld_seed5.log"
