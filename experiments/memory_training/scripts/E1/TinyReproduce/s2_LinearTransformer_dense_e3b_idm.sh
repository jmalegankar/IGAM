#!/usr/bin/env bash
# AUTO-GENERATED (E1). LinearTransformer/dense/e3b_idm seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/TinyReproduce/LinearTransformer_dense_e3b_idm.yaml"
echo "[E1] LinearTransformer/dense/e3b_idm seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LinearTransformer_dense_e3b_idm_seed2.log"
