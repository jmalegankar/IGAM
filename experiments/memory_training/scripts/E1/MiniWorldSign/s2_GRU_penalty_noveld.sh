#!/usr/bin/env bash
# AUTO-GENERATED (E1). GRU/penalty/noveld seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MiniWorldSign/GRU_penalty_noveld.yaml"
echo "[E1] GRU/penalty/noveld seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GRU_penalty_noveld_seed2.log"
