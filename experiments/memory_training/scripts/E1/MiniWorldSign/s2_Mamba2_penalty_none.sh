#!/usr/bin/env bash
# AUTO-GENERATED (E1). Mamba2/penalty/none seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MiniWorldSign/Mamba2_penalty_none.yaml"
echo "[E1] Mamba2/penalty/none seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Mamba2_penalty_none_seed2.log"
