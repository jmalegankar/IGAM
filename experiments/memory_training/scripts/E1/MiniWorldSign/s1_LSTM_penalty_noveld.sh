#!/usr/bin/env bash
# AUTO-GENERATED (E1). LSTM/penalty/noveld seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MiniWorldSign/LSTM_penalty_noveld.yaml"
echo "[E1] LSTM/penalty/noveld seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LSTM_penalty_noveld_seed1.log"
