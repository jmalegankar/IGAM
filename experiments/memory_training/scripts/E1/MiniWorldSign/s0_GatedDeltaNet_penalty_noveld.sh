#!/usr/bin/env bash
# AUTO-GENERATED (E1). GatedDeltaNet/penalty/noveld seed 0 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MiniWorldSign/GatedDeltaNet_penalty_noveld.yaml"
echo "[E1] GatedDeltaNet/penalty/noveld seed=0"
exec "$PYTHON" -m train --config "$CFG" --seed 0 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_penalty_noveld_seed0.log"
