#!/usr/bin/env bash
# AUTO-GENERATED (E1). GatedDeltaNet/aligned/none seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/MortarMayhem/GatedDeltaNet_aligned_none.yaml"
echo "[E1] GatedDeltaNet/aligned/none seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_aligned_none_seed3.log"
