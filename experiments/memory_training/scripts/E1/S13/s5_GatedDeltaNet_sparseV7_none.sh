#!/usr/bin/env bash
# AUTO-GENERATED (E1). GatedDeltaNet/sparseV7/none seed 5 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/S13/GatedDeltaNet_sparseV7_none.yaml"
echo "[E1] GatedDeltaNet/sparseV7/none seed=5"
exec "$PYTHON" -m train --config "$CFG" --seed 5 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_sparseV7_none_seed5.log"
