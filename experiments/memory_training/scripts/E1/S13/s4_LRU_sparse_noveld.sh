#!/usr/bin/env bash
# AUTO-GENERATED (E1). LRU/sparse/noveld seed 4 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/S13/LRU_sparse_noveld.yaml"
echo "[E1] LRU/sparse/noveld seed=4"
exec "$PYTHON" -m train --config "$CFG" --seed 4 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LRU_sparse_noveld_seed4.log"
