#!/usr/bin/env bash
# AUTO-GENERATED (E1). GRU/sparseV3/noveld seed 9 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/S13/GRU_sparseV3_noveld.yaml"
echo "[E1] GRU/sparseV3/noveld seed=9"
exec "$PYTHON" -m train --config "$CFG" --seed 9 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GRU_sparseV3_noveld_seed9.log"
