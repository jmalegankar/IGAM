#!/usr/bin/env bash
# AUTO-GENERATED (E1). RetNet/sparseV7/none seed 9 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../../configs/E1/S13/RetNet_sparseV7_none.yaml"
echo "[E1] RetNet/sparseV7/none seed=9"
exec "$PYTHON" -m train --config "$CFG" --seed 9 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/RetNet_sparseV7_none_seed9.log"
