#!/usr/bin/env bash
# AUTO-GENERATED (E5). GatedDeltaNet/besthp_sparse/e3b_idm seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[E5] GatedDeltaNet/besthp_sparse/e3b_idm seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_besthp_sparse_e3b_idm_seed2.log"
