#!/usr/bin/env bash
# AUTO-GENERATED (HPOT). GRU/penalty_oraclepot/none seed 0 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/HPOT/GRU_penalty_oraclepot_none.yaml"
echo "[HPOT] GRU/penalty_oraclepot/none seed=0"
exec "$PYTHON" -m train --config "$CFG" --seed 0 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GRU_penalty_oraclepot_none_seed0.log"
