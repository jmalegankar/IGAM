#!/usr/bin/env bash
# AUTO-GENERATED (HPOT). RetNet/penalty_oraclepot/none seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/HPOT/RetNet_penalty_oraclepot_none.yaml"
echo "[HPOT] RetNet/penalty_oraclepot/none seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/RetNet_penalty_oraclepot_none_seed2.log"
