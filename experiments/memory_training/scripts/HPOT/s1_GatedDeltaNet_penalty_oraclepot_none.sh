#!/usr/bin/env bash
# AUTO-GENERATED (HPOT). GatedDeltaNet/penalty_oraclepot/none seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[HPOT] GatedDeltaNet/penalty_oraclepot/none seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_penalty_oraclepot_none_seed1.log"
