#!/usr/bin/env bash
# AUTO-GENERATED (PBIM2S13). LSTM/sparseV7/pbim_e3b_idm seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM2S13/LSTM_sparseV7_pbim_e3b_idm.yaml"
echo "[PBIM2S13] LSTM/sparseV7/pbim_e3b_idm seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LSTM_sparseV7_pbim_e3b_idm_seed3.log"
