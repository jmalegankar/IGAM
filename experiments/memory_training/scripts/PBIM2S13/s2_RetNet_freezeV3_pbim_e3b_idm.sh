#!/usr/bin/env bash
# AUTO-GENERATED (PBIM2S13). RetNet/freezeV3/pbim_e3b_idm seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM2S13/RetNet_freezeV3_pbim_e3b_idm.yaml"
echo "[PBIM2S13] RetNet/freezeV3/pbim_e3b_idm seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/RetNet_freezeV3_pbim_e3b_idm_seed2.log"
