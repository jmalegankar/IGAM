#!/usr/bin/env bash
# AUTO-GENERATED (PBIM3S13). Memoryless/sparseV7/pbim_e3b_idm seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[PBIM3S13] Memoryless/sparseV7/pbim_e3b_idm seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Memoryless_sparseV7_pbim_e3b_idm_seed1.log"
