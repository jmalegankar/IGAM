#!/usr/bin/env bash
# AUTO-GENERATED (PBIM3S13). Mamba2/freezeV3/pbim_e3b_idm seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM3S13/Mamba2_freezeV3_pbim_e3b_idm.yaml"
echo "[PBIM3S13] Mamba2/freezeV3/pbim_e3b_idm seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Mamba2_freezeV3_pbim_e3b_idm_seed3.log"
