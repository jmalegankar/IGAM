#!/usr/bin/env bash
# AUTO-GENERATED (PBIM3MPG). GatedDeltaNet/penalty/pbim_e3b_idm seed 4 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM3MPG/GatedDeltaNet_penalty_pbim_e3b_idm.yaml"
echo "[PBIM3MPG] GatedDeltaNet/penalty/pbim_e3b_idm seed=4"
exec "$PYTHON" -m train --config "$CFG" --seed 4 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_penalty_pbim_e3b_idm_seed4.log"
