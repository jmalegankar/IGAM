#!/usr/bin/env bash
# AUTO-GENERATED (PBIM3MPG). RetNet/sparse/pbim_e3b_idm seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM3MPG/RetNet_sparse_pbim_e3b_idm.yaml"
echo "[PBIM3MPG] RetNet/sparse/pbim_e3b_idm seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/RetNet_sparse_pbim_e3b_idm_seed2.log"
