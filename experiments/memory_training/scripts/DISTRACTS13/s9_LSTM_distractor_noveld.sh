#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). LSTM/distractor/noveld seed 9 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/DISTRACTS13/LSTM_distractor_noveld.yaml"
echo "[DISTRACTS13] LSTM/distractor/noveld seed=9"
exec "$PYTHON" -m train --config "$CFG" --seed 9 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LSTM_distractor_noveld_seed9.log"
