#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). Mamba2/distractor/none seed 5 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/DISTRACTS13/Mamba2_distractor_none.yaml"
echo "[DISTRACTS13] Mamba2/distractor/none seed=5"
exec "$PYTHON" -m train --config "$CFG" --seed 5 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Mamba2_distractor_none_seed5.log"
