#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). Memoryless/distractor/noveld seed 5 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/DISTRACTS13/Memoryless_distractor_noveld.yaml"
echo "[DISTRACTS13] Memoryless/distractor/noveld seed=5"
exec "$PYTHON" -m train --config "$CFG" --seed 5 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Memoryless_distractor_noveld_seed5.log"
