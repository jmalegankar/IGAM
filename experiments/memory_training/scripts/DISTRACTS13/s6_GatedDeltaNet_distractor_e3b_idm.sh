#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). GatedDeltaNet/distractor/e3b_idm seed 6 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[DISTRACTS13] GatedDeltaNet/distractor/e3b_idm seed=6"
exec "$PYTHON" -m train --config "$CFG" --seed 6 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_distractor_e3b_idm_seed6.log"
