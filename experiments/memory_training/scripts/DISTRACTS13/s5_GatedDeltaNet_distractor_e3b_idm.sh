#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). GatedDeltaNet/distractor/e3b_idm seed 5 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[DISTRACTS13] GatedDeltaNet/distractor/e3b_idm seed=5"
exec "$PYTHON" -m train --config "$CFG" --seed 5 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_distractor_e3b_idm_seed5.log"
