#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). GatedDeltaNet/distractor/noveld seed 4 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[DISTRACTS13] GatedDeltaNet/distractor/noveld seed=4"
exec "$PYTHON" -m train --config "$CFG" --seed 4 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_distractor_noveld_seed4.log"
