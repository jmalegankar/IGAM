#!/usr/bin/env bash
# AUTO-GENERATED (DISTRACTS13). GatedDeltaNet/distractor/noveld seed 0 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[DISTRACTS13] GatedDeltaNet/distractor/noveld seed=0"
exec "$PYTHON" -m train --config "$CFG" --seed 0 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_distractor_noveld_seed0.log"
