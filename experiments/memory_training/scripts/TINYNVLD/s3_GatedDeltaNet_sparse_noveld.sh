#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). GatedDeltaNet/sparse/noveld seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/GatedDeltaNet_sparse_noveld.yaml"
echo "[TINYNVLD] GatedDeltaNet/sparse/noveld seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_sparse_noveld_seed3.log"
