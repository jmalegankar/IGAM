#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). LSTM/dense/noveld seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/LSTM_dense_noveld.yaml"
echo "[TINYNVLD] LSTM/dense/noveld seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/LSTM_dense_noveld_seed1.log"
