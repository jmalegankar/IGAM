#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). Mamba2/dense/noveld seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/Mamba2_dense_noveld.yaml"
echo "[TINYNVLD] Mamba2/dense/noveld seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Mamba2_dense_noveld_seed3.log"
