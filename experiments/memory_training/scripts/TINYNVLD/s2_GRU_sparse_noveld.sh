#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). GRU/sparse/noveld seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/GRU_sparse_noveld.yaml"
echo "[TINYNVLD] GRU/sparse/noveld seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GRU_sparse_noveld_seed2.log"
