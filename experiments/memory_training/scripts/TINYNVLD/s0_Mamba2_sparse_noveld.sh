#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). Mamba2/sparse/noveld seed 0 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/Mamba2_sparse_noveld.yaml"
echo "[TINYNVLD] Mamba2/sparse/noveld seed=0"
exec "$PYTHON" -m train --config "$CFG" --seed 0 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Mamba2_sparse_noveld_seed0.log"
