#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). RetNet/dense/noveld seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/RetNet_dense_noveld.yaml"
echo "[TINYNVLD] RetNet/dense/noveld seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/RetNet_dense_noveld_seed3.log"
