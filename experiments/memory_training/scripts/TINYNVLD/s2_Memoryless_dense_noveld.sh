#!/usr/bin/env bash
# AUTO-GENERATED (TINYNVLD). Memoryless/dense/noveld seed 2 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/TINYNVLD/Memoryless_dense_noveld.yaml"
echo "[TINYNVLD] Memoryless/dense/noveld seed=2"
exec "$PYTHON" -m train --config "$CFG" --seed 2 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Memoryless_dense_noveld_seed2.log"
