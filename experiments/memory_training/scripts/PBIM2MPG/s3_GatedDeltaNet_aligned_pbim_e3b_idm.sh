#!/usr/bin/env bash
# AUTO-GENERATED (PBIM2MPG). GatedDeltaNet/aligned/pbim_e3b_idm seed 3 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
echo "[PBIM2MPG] GatedDeltaNet/aligned/pbim_e3b_idm seed=3"
exec "$PYTHON" -m train --config "$CFG" --seed 3 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_aligned_pbim_e3b_idm_seed3.log"
