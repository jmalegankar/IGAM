#!/usr/bin/env bash
# AUTO-GENERATED (PBIM2MPG). GatedDeltaNet/penalty/pbim_e3b_idm seed 0 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM2MPG/GatedDeltaNet_penalty_pbim_e3b_idm.yaml"
echo "[PBIM2MPG] GatedDeltaNet/penalty/pbim_e3b_idm seed=0"
exec "$PYTHON" -m train --config "$CFG" --seed 0 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/GatedDeltaNet_penalty_pbim_e3b_idm_seed0.log"
