#!/usr/bin/env bash
# AUTO-GENERATED (PBIM2MPG). Mamba2/penalty/pbim_e3b_idm seed 1 — ONE run / one GPU, snapshots ON.
set -euo pipefail
...
CFG="$HERE/../../configs/PBIM2MPG/Mamba2_penalty_pbim_e3b_idm.yaml"
echo "[PBIM2MPG] Mamba2/penalty/pbim_e3b_idm seed=1"
exec "$PYTHON" -m train --config "$CFG" --seed 1 \
    --runs-dir "$RUNS_DIR" --device "$DEVICE" \
    --wandb --snapshot-steps "$SNAP" $SNAP_WANDB $EXTRA \
    2>&1 | tee "$LOGDIR/Mamba2_penalty_pbim_e3b_idm_seed1.log"
