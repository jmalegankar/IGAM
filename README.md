...
Every cell implements the [`RecurrentCell`](igam/cell/base.py) interface — `init_state`, `step`, `forward_sequence`, `reset_state` — and is drop-in interchangeable in the trainer.

| Cell | File | Reference |
| --- | --- | --- |
| GRU | [gru.py](igam/cell/gru.py) | Cho et al. 2014 |
| LSTM | [lstm.py](igam/cell/lstm.py) | Hochreiter & Schmidhuber 1997 |
| LMU | [lmu.py](igam/cell/lmu.py) | Voelker et al. 2019 |
| Linear Transformer | [linear_transformer.py](igam/cell/linear_transformer.py) | Katharopoulos et al. 2020 |
| RetNet | [retnet.py](igam/cell/retnet.py) | Sun et al. 2023 |
| DeltaNet | [deltanet.py](igam/cell/deltanet.py) | Schlag 2021; Yang et al. 2024 |
| S4D | [s4d.py](igam/cell/s4d.py) | Gu et al. 2022 |
| Mamba2 | [mamba2.py](igam/cell/mamba2.py) | Dao & Gu 2024 |
| mLSTM | [mlstm.py](igam/cell/mlstm.py) | Beck et al. 2024 (xLSTM) |
| SHM | [shm.py](igam/cell/shm.py) | Le et al. 2024 (Stable Hadamard Memory) |

Test-friendly hyperparameter defaults for every cell live in [`igam/cell/tests/conftest.py`](igam/cell/tests/conftest.py); CLI defaults for `train.py` live in [`DEFAULT_CELL_KWARGS`](train.py).

## Supported environment suites

[`make_vec_env(env_name, n_envs, seed)`](igam/envs/__init__.py) dispatches on the env id and returns an SB3 `VecEnv` ready for the trainer.

| Suite | Env-id pattern | Wrapper |
| --- | --- | --- |
| [POPGym](https://github.com/proroklab/popgym) | `popgym-*-v0` | [popgym_wrappers.py](igam/envs/popgym_wrappers.py) |
| [MiniGrid](https://github.com/Farama-Foundation/Minigrid) | `MiniGrid-*` | [minigrid_wrappers.py](igam/envs/minigrid_wrappers.py) |
| [memory-gym](https://github.com/MarcoMeter/endless-memory-gym) | `MortarMayhem*`, `MysteryPath*`, `SearingSpotlights*`, `Endless-*` | [memory_gym_wrappers.py](igam/envs/memory_gym_wrappers.py) |
| [popgym-arcade](https://github.com/bolt-research/popgym-arcade) | `popgym-arcade-*` | [popgym_arcade_wrappers.py](igam/envs/popgym_arcade_wrappers.py) |
...
- **Encoder split.** Optional separate-backbone actor/critic (`separate_backbones=True` in `IGAMPPO`) — useful when a shared encoder bottlenecks one head.
